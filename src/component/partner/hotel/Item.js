...
import ModalUploadRoom from './ModalUploadRoom';
export default function Item({ data, index, search }) {
    ...
    let typeHotelName = typeHotels.filter((item) => item.value == data?.type)?.[0]?.name;
    let approveStatusName = approveStatus.filter((item) => item.value == data?.approveStatus)?.[0]?.name;
    ...
    let deleteItem = async () => {
        let message = window.confirm("Bạn chắc chắn muốn xóa khách sạn này?");
        if (message) {
            let path = `/partner/hotel/delete/${data?.code}`;
    ...
    return (
        <>
            <tr >
                ...
                <td className="text-center" style={{ verticalAlign: 'middle' }}>{data?.minPrice} VNĐ</td>
                ...
                <ModalUpdate handleClose={handleCloseUpdate} handleShow={handleShowUpdate} show={showUpdate} data={data} images={hotel?.images} search={search}
                    hotelCode={data?.code}
                />
                ...
                <td style={{ verticalAlign: 'middle' }} className="btn__setting">
                    ...
                    <ModalDetail handleClose={handleCloseDetail} handleShow={handleShowDetail} show={showDetail} slug={data?.slug} isUpload={isUpload}
                        typeHotelName={typeHotelName}
                        star={data?.star}
                        rank={data?.rank}
                    />
                    <ModalUploadRoom search={search} handleClose={handleClose} handleShow={handleShow} show={show} hotelCode={data?.code} handleUpload={handleUpload} />
