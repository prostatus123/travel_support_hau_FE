...
export default function Hotel() {
    ...
    let searchFavourite = async () => {
        ...
        if (resp.ok) {
            let resps = [];
            let response = await resp.json();
            for (let i = 0; i < response?.data?.hotel?.length; i++) {
                resps.push(response?.data?.hotel[i]?.slug)
            }
            ...
            <div className="wrapper__search search__hotel container">
                ...
                <div className="row row__search-page">
                    <div className="col-2 col__filter">
                        <Filter
                            star={star} type={type}
                            handleStar={handleStar} handleType={handleType}
                        ...
                        <div className="wrapper__search container no-mg">
                            ...
                            {data?.hotels?.length > 0 ?
                                data?.hotels?.map((item, index) => {
                                    return (
                                        <Item key={index} image={item?.image} slug={item?.slug} favourite={favourite} searchFavourite={searchFavourite} />
                                    )
                                }) :
                                <>
                                    Không có dữ liệu
